...
def preprocess_function_summ_iwslt(data, tgt_lang, src_context_size, tgt_context_size, num_summary_sentences, prompt_type, api, max_length, summarized_contexs):
    ...
    if src_context_size >= 1:
        src_intersec = [sent for doc_idx, doc in enumerate(data["doc"]) for sent in doc["en"]]
             
    elif tgt_context_size >= 1:
        src_intersec = [sent for doc_idx, doc in enumerate(data["doc"]) for sent in doc[tgt_lang]]
