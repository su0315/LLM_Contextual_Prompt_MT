...
def generate_context(src_context_size, prompt_talk_id, data):
    ...
    for doc_idx, doc in enumerate(data["doc"]):
        doc_input = [sent for doc in data["doc"][prompt_talk_index+1:3] for sent in doc["en"][:10]] # 10 sentences from each document
...
def preprocess_function(src_context_size, tgt_lang, model_checkpoint, prompt, prompt_talk_id, max_length, tokenizer, data): # data should be splitted into train / dev / test internally
    ...
    if src_context_size >= 1:
        print ("Yes2")
        for doc_idx, doc in enumerate(data["doc"]):
            doc_input = [sent for doc in data["doc"][prompt_talk_index+1:] for sent in doc["en"]] # 10 sentences from each document
        ...
        inputs = [prompt + ip + after_ip for doc in data["doc"][prompt_talk_index+1:] for sent in doc["en"]] 
