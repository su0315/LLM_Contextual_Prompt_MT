...
def main():
    ...
    inputs = tokenizer(prompt + "Last year at TED I gave an introduction to the LHC. = ", return_tensors="pt").input_ids
    print ("inputs", prompt, "Last year at TED I gave an introduction to the LHC. = ")
    model = XGLMForCausalLM.from_pretrained(model_checkpoint)
    outputs = model.generate(inputs, max_new_tokens=100, do_sample=True, top_k=50, top_p=0.95) # 
    prediction = tokenizer.batch_decode(outputs, skip_special_tokens=True)
    print ("One sentence prediction", prediction)
    """
    # Parameters
    tgt_lang = "ja"
    data_path = "/home/sumire/thesis/LLM_Contextual_Prompt_MT/data/iwslt_hf/"
